...
describe("GET /", () => {
  it("Should get all cities with added density field calculated and added to the response object", async () => {
...
describe("GET /sort/:property/:order", () => {
  ...
  const secondRandomNum = Math.floor(
    Math.random() * (data.length - 1 - (firstRandomNum + 1)) +
      (firstRandomNum + 1)
  );
  ...
  it("Should get sorted cities by name in ascending order", async () => {
    ...
    expect(
      response.body[firstRandomNum].name <= response.body[secondRandomNum].name
    ).toBe(true);
  });
  ...
  it("Should get sorted cities by area in ascending order", async () => {
    ...
    expect(response.body).not.toEqual(data);
    expect(
      response.body[firstRandomNum].area <= response.body[secondRandomNum].area
    ).toBe(true);
  });

  it("Should get sorted cities by population in ascending order", async () => {
    const response = await supertest(app)
      .get("/sort/population/asc")
      .expect(200);
    expect(response.body).not.toEqual(data);
    expect(
      response.body[firstRandomNum].population <=
        response.body[secondRandomNum].population
    ).toBe(true);
  });
  ...
  it("Should get error if sort property is not valid", async () => {
    const expectedError = {
      error: BAD_REQUEST,
      message: INVALID_SORT_PROPERTY_ERR_MSG,
    };
    await supertest(app)
      .get("/sort/test/desc")
      .expect(400)
  ...
  it("Should get error if sort order is not valid", async () => {
    const expectedError = {
      error: BAD_REQUEST,
      message: INVALID_SORT_ORDER_ERR_MSG,
    };
    await supertest(app)
      .get("/sort/name/test")
      .expect(400)
